...
// Quick Sort
  void quickSort(List<int> numbers, int low, int high) async {
    startSorting();
    await _quickSort(numbers, low, high);
    if (isCancelled) {
      cancelledSorting();
    } else {
      finishedSorting();
  ...
  Future<void> _quickSort(List<int> numbers, int low, int high) async {
    if (low < high) {
      int pivot = await _partition(numbers, low, high);
      await _quickSort(numbers, low, pivot - 1);
      await _quickSort(numbers, pivot + 1, high);
    }
  }
  ...
  Future<int> _partition(List<int> numbers, int low, int high) async {
    ...
    // Indicate checking pivot
    setUpdateText('Checking pivot: ${numbers[high]}');
    await Future.delayed(const Duration(seconds: 1));
    ...
    for (int j = low; j <= high - 1; j++) {
      ...
      if (numbers[j] < pivot) {
        i++;
        swap(numbers, i, j);
        updatePointers([i, j, high]);
        setUpdateText('Swapping ${numbers[i]} and ${numbers[j]}');
        await Future.delayed(const Duration(seconds: 1));
      }
    }

    // Swapping pivot with the correct position
    swap(numbers, i + 1, high);
    updatePointers([i + 1, high]);
    setUpdateText('Swapping ${numbers[i + 1]} and ${numbers[high]}');
    await Future.delayed(const Duration(seconds: 2));
